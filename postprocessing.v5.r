...
spec = matrix(c(
    ...
    "node.list"   ,  "n" , 1, "character","List (\\n separated) of nodes to be reported on (aka input species/node list used for RMAex)."
), byrow=TRUE, ncol=5);
opt = getopt(spec);
...
image(x=1:ncol(red.res),y=1:nrow(red.res),z=t(red.res),col=mycol,axes=F,ylab="",xlab="")
...
for (spl in colnames(red.res)){
    for (tax in rownames(red.res)){
        if( red.res[tax,spl] > 1 ){
            ...
            par(mfrow=c(3,2))
            plot.editDis.perID(spl,tax,folder.names)
            plot.mapDamage2(spl,tax,folder.names[1])
            plot.readDisTable3(spl,tax,folder.names[1])
            dev.off()
        }
    }
}
